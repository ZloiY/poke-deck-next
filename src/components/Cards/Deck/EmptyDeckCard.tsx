...
export const EmptyDeckCard = memo(
  ({
    ...
    <BlankDeckCard className={className} onClick={() => addCard?.(deck.id)} notInteractive={notInteractive}>
      {deck.private && (
          <Private className={twMerge("absolute top-2 left-1 w-10 h-10", notInteractive && "top-40 left-0")} />
        )}
      <div className="relative flex justify-center items-center h-full w-full">
        ...
        <p className={twMerge("absolute top-0 font-coiny text-3xl", notInteractive && 'text-base')}>{deck.name}</p>
